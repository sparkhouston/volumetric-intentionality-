...
export default function App() {
  ...
  const TaskList = ({pillar}) => {
    ...
    const DailyRow = ({item,onToggle}) => (
      <div onClick={onToggle} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 14px",marginBottom:4,background:item.done?"#f9f9f9":"#fff",border:`1px solid ${item.done?"#ddd":"#f0f0f0"}`,borderRadius:isH?8:0,cursor:"pointer"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            ...
            <div style={{display:"flex",gap:6}}>
              {[["once","One & Done"],["daily","Daily"]].map(([v,l])=>(
                <button key={v} onClick={()=>setNewType(v)} style={{fontSize:9,padding:"5px 12px",cursor:"pointer",background:newType===v?(isH?"#1a56db":"#000"):"transparent",color:newType===v?"#fff":"#aaa",border:`1px solid ${newType===v?(isH?"#1a56db":"#000"):"#e8e8e8"}`,borderRadius:isH?20:0}}>{l}</button>
              ))}
            </div>
  ...
  const GoalsScreen = () => {
    const [newGoal, setNewGoal] = useState({text:"",pillar:"earth",horizon:"6month",notes:""});
    const [showAdd, setShowAdd] = useState(false);
    const addGoal = () => {
      if (!newGoal.text.trim()) return;
      setGoals(prev => [...prev, {id:`g${uid++}`, ...newGoal, progress:0}]);
      setNewGoal({text:"",pillar:"earth",horizon:"6month",notes:""});
      setShowAdd(false);
    };
    ...
    return (
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        <div className={cardA} style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div className={`df screen-title ${isH?"heaven-title":"earth-title"}`} style={{fontSize:20}}>North Star Goals</div>
            <div style={{fontSize:10,color:"#000",fontWeight:600,marginTop:2}}>Big picture. Long game.</div>
          </div>
          <button className={btnS} onClick={()=>setShowAdd(!showAdd)} style={{whiteSpace:"nowrap"}}>{showAdd?"CANCEL":"+ GOAL"}</button>
        </div>
        {showAdd&&(
          <div className={cardA}>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              <input className={iCls} placeholder="The goal. Make it real." value={newGoal.text} onChange={e=>setNewGoal(p=>({...p,text:e.target.value}))} style={{fontSize:16}}/>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                <select className={iCls} value={newGoal.pillar} onChange={e=>setNewGoal(p=>({...p,pillar:e.target.value}))}>
                  <option value="earth">Earth</option>
                  <option value="heaven">Heaven</option>
                  <option value="balance">Balance</option>
                </select>
                <select className={iCls} value={newGoal.horizon} onChange={e=>setNewGoal(p=>({...p,horizon:e.target.value}))}>
                  {HORIZONS.map(h=><option key={h.id} value={h.id}>{h.label}</option>)}
                </select>
              </div>
              <input className={iCls} placeholder="Notes (optional)" value={newGoal.notes} onChange={e=>setNewGoal(p=>({...p,notes:e.target.value}))} style={{fontSize:16}}/>
              <button className={btnS} onClick={addGoal}>ADD GOAL</button>
            </div>
          </div>
        )}
        {goals.map(g=>{
          const related = relatedTasks(g);
          return (
            <div key={g.id} className={card} style={{borderLeft:`3px solid ${pillarColor(g.pillar)}`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10,marginBottom:10}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:8,letterSpacing:"0.2em",color:pillarColor(g.pillar),fontWeight:700,marginBottom:4,textTransform:"uppercase"}}>{g.pillar} · {horizonLabel(g.horizon)}</div>
                  <div className="df" style={{fontSize:17,fontWeight:600,lineHeight:1.3,color:"#000"}}>{g.text}</div>
                  {g.notes&&<div style={{fontSize:10,color:"#888",marginTop:4}}>{g.notes}</div>}
                </div>
                <button onClick={()=>deleteGoal(g.id)} style={{background:"none",border:"none",color:"#ccc",cursor:"pointer",fontSize:16,flexShrink:0}}>×</button>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                <div style={{flex:1,height:3,background:"#eee"}}>
                  <div style={{height:"100%",width:`${g.progress}%`,background:pillarColor(g.pillar),transition:"width 0.4s"}}/>
                </div>
                <span className="df" style={{fontSize:20,fontWeight:700,color:pillarColor(g.pillar),width:40,textAlign:"right"}}>{g.progress}%</span>
              </div>
              <div style={{display:"flex",gap:6,marginBottom:related.length>0?12:0,flexWrap:"wrap"}}>
                {[0,25,50,75,100].map(v=>(
                  <button key={v} onClick={()=>updateProgress(g.id,v)} style={{fontSize:9,padding:"4px 10px",cursor:"pointer",background:g.progress===v?pillarColor(g.pillar):"transparent",color:g.progress===v?"#fff":"#aaa",border:`1px solid ${g.progress===v?pillarColor(g.pillar):"#e8e8e8"}`,borderRadius:isH?20:0}}>{v}%</button>
                ))}
              </div>
              {related.length>0&&(
                <div style={{borderTop:"1px solid #f0f0f0",paddingTop:10}}>
                  <div style={{fontSize:8,letterSpacing:"0.2em",color:"#aaa",fontWeight:600,marginBottom:6}}>FEEDING THIS GOAL</div>
                  {related.map(t=>(
                    <div key={t.id} style={{fontSize:11,color:"#000",padding:"3px 0",display:"flex",gap:8}}>
                      <span style={{fontSize:8,color:pillarColor(g.pillar)}}>→</span>
  ...
  const TABS = [
    ...
    {id:"goals",label:"North Star"},
    {id:"schedule",label:"Schedule"},
  ];

  const tabTitles = {dump:"Mind Dump",overview:"Overview",earth:"Earth",heaven:"Heaven",balance:"Balance",goals:"North Star",schedule:"Schedule"};
  const tabSubs = {dump:"What's on your mind?",overview:"Your empire at a glance",earth:"Work & Business",heaven:"Creative & Music",balance:"Life, Health & People",goals:"Big picture. Long game.",schedule:"Your daily architecture"};
      ...
      <div style={{position:"relative",zIndex:1}}>
        <div style={{padding:"18px 16px 0",maxWidth:680,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div className="df" style={{fontSize:"clamp(11px,2.5vw,14px)",fontWeight:300,letterSpacing:"0.08em",color:"#aaa"}}>Volumetric Intentionality</div>
            <div style={{fontSize:9,color:"#000",letterSpacing:"0.15em"}}>{now.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})} · {isH?"Heaven":"Earth"}</div>
          </div>
        </div>
        <div style={{maxWidth:680,margin:"12px auto 0",position:"sticky",top:0,zIndex:10,background:"rgba(255,255,255,0.97)",backdropFilter:"blur(8px)"}}>
          <div className={`nav ${isH?"heaven-nav":"earth-nav"}`}>
            {TABS.map(t=>(
              <button key={t.id} className={`ntab ${isH?"heaven-ntab":"earth-ntab"} ${tab===t.id?"active":""}`} onClick={()=>setTab(t.id)}>{t.label}</button>
            ))}
          </div>
        </div>
