...
const CSS = `
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  /* INPUTS */
  input, select, textarea { font-family:'DM Mono',monospace; font-size:12px; padding:11px 14px; outline:none; width:100%; transition:border 0.2s; }
  ...
  .hfab { background:rgba(26,86,219,0.85); color:#fff; box-shadow:0 2px 24px rgba(26,86,219,0.3);  }
  ...
  .alarm-box { max-width:360px; width:100%; padding:36px 28px; text-align:center;  }
  .earth-alarm { background:#fff; border:2px solid #000; }
  .heaven-alarm { background:#fafaf8; border:1px solid rgba(26,86,219,0.5); border-radius:16px;  }
  ...
  /* SCREEN */
  .screen {  padding:20px 16px 100px; max-width:680px; margin:0 auto; }
...
// ─── ISOLATED CLOCK (won't cause app re-renders) ──────────────────────────────
const LiveClock = memo(function LiveClock({ style }) {
  const [t, setT] = useState(new Date());
  useEffect(() => {
    // Update every minute only - not every second
    const interval = setInterval(() => setT(new Date()), 60000);
...
const LiveDate = memo(function LiveDate({ style }) {
  const [t, setT] = useState(new Date());
  useEffect(() => {
    const interval = setInterval(() => setT(new Date()), 60000);
    return () => clearInterval(interval);
...
export default function App() {
  ...
  // currentTime removed — now in isolated Clock component
  ...
  // Mode + alarms only (clock is in its own component)
  useEffect(() => {
    const t = setInterval(() => {
      const n = new Date();
...
Only include <tasks> for clear actionable items, not for pure conversation or questions.`;
  ...
  const handleDumpSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method:"POST",
        headers:{ "Content-Type":"application/json", "x-api-key":"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", "anthropic-version":"2023-06-01", "anthropic-dangerous-direct-browser-access":"true" },
        body: JSON.stringify({
      ...
      <div className="dump-messages">
          ...
          <div key={m.id} className={`msg msg-${m.role}`}>
            ...
            {m.extracted && m.extracted.length > 0 && (
              <div style={{marginTop:8, display:"flex", flexWrap:"wrap", gap:6}}>
                {m.extracted.map((t,i) => (
                  <div key={i} style={{fontSize:9, padding:"3px 10px", background: t.pillar==="earth"?"#f0f0f0":t.pillar==="heaven"?"rgba(26,86,219,0.1)":"rgba(0,160,100,0.08)", border:`1px solid ${t.pillar==="earth"?"#ddd":t.pillar==="heaven"?"rgba(26,86,219,0.3)":"rgba(0,160,100,0.2)"}`, color: t.pillar==="earth"?"#666":t.pillar==="heaven"?"#1a56db":"#2a7a50", borderRadius:20, letterSpacing:"0.05em"}}>
        ...
        {aiLoading && (
          <div className="msg msg-ai">
            <div className={`msg-bubble ${isH?"heaven-ai-bubble":"earth-ai-bubble"}`} style={{}}>
      ...
      <div style={{position:"sticky",bottom:80,paddingTop:12}}>
        <div style={{display:"flex",gap:8}}>
          <input
            ...
            onKeyDown={e=>e.key==="Enter"&&handleDumpSubmit()}
            disabled={aiLoading}
            autoFocus
          />
          <button className={btnS} onClick={handleDumpSubmit} disabled={aiLoading} style={{whiteSpace:"nowrap",padding:"11px 18px",opacity:aiLoading?0.5:1}}>→</button>
        </div>
  ...
  const ScheduleScreen = () => {
    ...
    useEffect(() => {
      const t = setInterval(() => setSchedTime(new Date()), 60000);
      return () => clearInterval(t);
    }, []);
    const currentTime = schedTime;
    ...
    <div className={`app ${isH?"heaven-app":"earth-app"}`}>
      ...
      {isH && (
        <div className="heaven-bg-layer">
          <div style={{position:"absolute",top:"-10%",left:"50%",transform:"translateX(-50%)",width:"60vw",height:"60vw",maxWidth:400,maxHeight:400,borderRadius:"50%",background:"radial-gradient(circle,rgba(191,219,254,0.12) 0%,rgba(147,197,253,0.05) 40%,transparent 70%)"}} />
      ...
      {confetti && (
        <div style={{position:"fixed",inset:0,zIndex:999,pointerEvents:"none"}}>
          {[...Array(40)].map((_,i)=>(
            <div key={i} style={{position:"absolute",left:`${Math.random()*100}%`,top:"-8px",width:`${4+Math.random()*5}px`,height:`${4+Math.random()*5}px`,background:isH?["#bfdbfe","#93c5fd","#3b82f6","#fff","#dbeafe"][Math.floor(Math.random()*5)]:["#111","#555","#888","#ccc","#333"][Math.floor(Math.random()*5)],borderRadius:Math.random()>0.5?"50%":"1px",}s ease-in ${Math.random()*0.5}s forwards`}} />
          ))}
        </div>
      )}
